...
# fill NA with mean
df_eng$Data <- ifelse(is.na(df_eng$Data), mean(df_eng$Data, na.rm=TRUE), df_eng$Data)
...
ggplot(df_eng, aes(x=Day, y=Data))+
  geom_line()+
  labs(x="Day", y=y_axis_title, title=graph_title) +
  theme_classic()
ggsave(paste(output_folder,"//","1basic_graph1.png",sep=""),width=30,height=15,units="cm")
...
autoplot(df_eng,Data) +
  labs(title = graph_title,
       y=y_axis_title) +
  theme_classic()
ggsave(paste(output_folder,"//","2basic_graph2.png",sep=""),width=30,height=15,units="cm")
...
ggsave(paste(output_folder,"//","3differences1.png",sep=""),width=30,height=15,units="cm")

# get ACF and PACF plots
df_eng |> ACF(difference(Data)) |> autoplot() +
  theme_classic()
ggsave(paste(output_folder,"//","4differences1_acf.png",sep=""),width=30,height=15,units="cm")

df_eng |> PACF(difference(Data)) |> autoplot() +
  theme_classic()
ggsave(paste(output_folder,"//","5differences1_pacf.png",sep=""),width=30,height=15,units="cm")
...
ggsave(paste(output_folder,"//","6differences2.png",sep=""),width=30,height=15,units="cm")

# get ACF and PACF plots
df_eng |> ACF(difference(Data) |> difference()) |> autoplot() +
  theme_classic()
ggsave(paste(output_folder,"//","7differences2_acf.png",sep=""),width=30,height=15,units="cm")

df_eng |> PACF(difference(Data) |> difference()) |> autoplot() +
  theme_classic()
ggsave(paste(output_folder,"//","8differences2_pacf.png",sep=""),width=30,height=15,units="cm")
...
ggplot(model_perf, aes(x=.model, y=MAPE)) +
  ...
  theme_classic()
ggsave(paste(output_folder,"//","9model_rmse.png",sep=""),width=30,height=15,units="cm")

################################################################################

# forecast all models
forecast(fit_train, h=rows_to_use) %>%
  mutate(Data = distributional::dist_truncated(Data, lower = 0)) %>%
  autoplot(train, level = NULL) + # NULL level to not show confidence intervals
  autolayer(test, color="black") +
  geom_rect(aes(xmin = df_eng$Day[nrow(df_eng)-rows_to_use+1], xmax = df_eng$Day[nrow(df_eng)], ymin = -Inf, ymax = Inf),
            fill = "#CECECE", alpha = 0.03)+
  guides(colour = guide_legend(title = "Forecast")) +
  labs(title = graph_title, y=y_axis_title) +
  theme_classic()
ggsave(paste(output_folder,"//","10forecast_all.png",sep=""),width=30,height=15,units="cm")
...
forecast(fit, h=36) %>%
  ...
  labs(title = paste(graph_title,", ARIMA(",model_res1$p_val, d_spec,model_res1$q_val,")+(",model_res1$P_val, d_spec,model_res1$Q_val,") model", sep=""),
       y=y_axis_title) +
  theme_classic()
ggsave(paste(output_folder,"//","11forecast_iter.png",sep=""),width=30,height=15,units="cm")
...
ggsave(paste(output_folder,"//","12residuals.png",sep=""),width=30,height=15,units="cm")
...
forecast(fit, h=36) %>%
  ...
  labs(title = paste(graph_title,", ARIMA model", sep=""),
       y=y_axis_title) +
  theme_classic()
ggsave(paste(output_folder,"//","13forecast.png",sep=""),width=30,height=15,units="cm")
...
df_stl %>%
  ...
  labs(title = graph_title,
       y=y_axis_title) +
  theme_classic()
ggsave(paste(output_folder,"//","14bootstrap_generate.png",sep=""),width=30,height=15,units="cm")
...
ets_forecasts <- sim %>%
  model(ets = ETS(.sim)) %>%
  forecast(h = 12) %>%
  mutate(.sim = distributional::dist_truncated(.sim, lower = 0))

ets_forecasts %>%
  update_tsibble(key = .rep) %>%
  autoplot(.mean) +
  autolayer(df_eng, Data) +
  #geom_rect(aes(xmin = Day[nrow(ets_forecasts)-12], xmax = Day[nrow(ets_forecasts)], ymin = -Inf, ymax = Inf),
  #          fill = "#CECECE", alpha = 0.03)+
  guides(colour = "none") +
  labs(title = paste(graph_title,": bootstrapped forecasts", sep=""),
       y=y_axis_title) +
  theme_classic()
ggsave(paste(output_folder,"//","15bootstrap_forecast.png",sep=""),width=30,height=15,units="cm")
...
df_eng %>%
  ...
  labs(title = paste(graph_title,", ETS model from bootstrapped averages", sep=""),
       y=y_axis_title) +
  theme_classic()
ggsave(paste(output_folder,"//","16bagged_bootstrap_forecast.png",sep=""),width=30,height=15,units="cm")