...
for (run in 1:n_runs){
  ...
  if (teams[h] != teams[a]){
    ...
    h_team_prob <- team_probs[h] * play_prob[1]
    #outcome_prob_h = [x*h_team_prob for x in outcome_prob]
    a_team_prob <- team_probs[a] * play_prob[2]
    ...
    if (result == "W"){
      ...
      team_probs[h] <- min(0.95,(1+win_chg[1]) * team_probs[h])
      team_probs[a] <- max(0.01,(1+loss_chg[1]) * team_probs[a])
    }
    else if (result == "L"){
      ...
      team_probs[h] <- max(0.01,(1+loss_chg[1]) * team_probs[h])
      team_probs[a] <- min(0.95,(1+win_chg[1]) * team_probs[a])
    }
    else{
    ...
    # outcome probability
    dff <- data.frame(run=run, home=teams[h], away=teams[a], probs=paste(paste(outcome_prob_h,sep=" "),collapse=''), result=result, score=paste(h_score,":",a_score,sep=""),goals_h=h_score, goals_a=a_score, exp_result=exp_result,
